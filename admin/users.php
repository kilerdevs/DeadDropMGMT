...
try {
    $db = get_db();
    $owner = $db->query('SELECT id, username, created_at FROM users WHERE role = "owner" LIMIT 1')->fetch();
    $couriers = $db->query(
        'SELECT u.id, u.username, u.created_at, COUNT(o.id) AS order_count
         FROM users u
    ...
    <main class="main">
        ...
        <div class="table-wrap">
            <table>
                ...
                <tr>
                    <td><span class="token"><?= htmlspecialchars($c['username'], ENT_QUOTES, 'UTF-8') ?></span></td>
